...
class NTU_RGBD_VIDEO_Dataset(data.Dataset):
    ...
    def __init__(self, mode='train', data_list=None, debug=False, extract_frame=1, transform=None, target_transform=None):
        ...
        '''
        super(NTU_RGBD_VIDEO_Dataset, self).__init__()
        ...
        if gl.dataset == 'ntu120_30':
            ...
            path = '/mnt/data1/kinetics-skeleton/random_data/train_val_test_100'
            segment = 50
        else:
            path = None
            segment = 0
    ...
    def extract_frame(self, x, num_frame, segment, leave_all_frame=False):
        ...
        if leave_all_frame > 0:
            num_frame = np.ones(n) * 300
        ...
        for i in range(n):
            if num_frame[i] < segment:
                new_x.append(np.expand_dims(x[i, :, 0:segment, :, :], 0).reshape(1, c, segment, h, w))
                continue
            idx = [random.randint(j * step[i], (j + 1) * step[i] - 1) for j in range(segment)]
            idxs.append(idx)
            new_x.append(np.expand_dims(x[i, :, idx, :, :], 0).reshape(1, c, segment, h, w))
