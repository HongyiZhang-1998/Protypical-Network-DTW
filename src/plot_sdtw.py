...
parse.add_argument('--data_dir', type=str, default="/home/zhanghongyi/dtw-prototypical-network/vc/2s_AGCN/ntu120/diagonal_reg")
parse.add_argument('--category', type=str, default="D_")
...
path = os.path.join(data_path, 'epoch{}_iter_1100.npy'.format(plot_epoch))
data = np.load(path)
n, _, _ = data.shape
...
for j in range(0, 20):
    ...
    ax.set_ylabel('')
    s2 = p2.get_figure()
    single = os.path.join(save_path, 'single_' + 'epoch{}_iter_1100'.format(plot_epoch))
