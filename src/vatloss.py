...
class VATLoss(nn.Module):
    ...
    def forward(self, model, x, y, pred_loss):
        with torch.no_grad():
            # pred = F.softmax(pred_loss, dim=0)
            pred = pred_loss

        # prepare random unit tensor
        d =  (torch.rand(x.shape[2], x.shape[2]).to(x.device)) # T * T
        d = _l2_normalize(d)
        # print('d', d)
        ...
        # calc adversarial direction
            for _ in range(self.ip):  #3-5
                ...
                _, _, _, pred_hat = model.loss(x, y, gl.shot, gl.dtw, d=d)
            ...
            # print('r_adv', r_adv)
            _, _, _, pred_hat = model.loss(x, y, gl.shot, gl.dtw, d=r_adv)
            # logp_hat = F.log_softmax(pred_hat, dim=0)
